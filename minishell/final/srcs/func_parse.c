...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/10 17:45:10 by hyeokim           #+#    #+#             */
/*   Updated: 2021/01/10 18:11:59 by hyeokim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		set_quote(char *argv, int idx, t_quote *q)
{
	if (q->type == INIT)
	{
		if (argv[idx] == '\'')
...
char			*parse_env(char *argv, t_list *envs)
{
	...
	while (argv[++idx])
	{
		if (argv[idx] == '$' || argv[idx + 1] == '\0')
		{
			...
			temp[1] = ft_strdup(get_value(temp[0], envs));
...
char			*parse_quote(char *argv, t_list *envs)
{
	...
	while (argv[++idx])
	{
		set_quote(argv, idx, &q);
		if (q.end != INIT)
		{
			tmp[1] = ft_substr(argv, q.start, q.end - q.start + 1);
			if (q.type == DQUOTE && check_env(tmp[1]))
